...
if [[ "\$args" == *"/app/installations"* && "\$args" != *"access_tokens"* ]]; then
  ...
  printf '[{"name":"agent-gh-access-apps-blocked-from-non-ai-branches"},{"name":"agent-gh-access-apps-must-sign"}]'
elif [[ "\$args" == *"inventory---internal-do-not-delete"* && "\$args" == *"/git/ref/heads/"* ]]; then
  # Branch existence check (no --fail-with-body)
  if [[ "${branch_state}" == "branch-exists" ]]; then
    printf '{"ref":"refs/heads/x-ai/${TEST_OWNER}/inventory---internal-do-not-delete","object":{"sha":"invbranchsha"}}'
  else
    printf '{}'
  fi
elif [[ "\$args" == *"inventory---internal-do-not-delete"* && "\$args" == *"/git/trees/"* ]]; then
  # Tree listing for file deletion during init — return empty tree (no files to delete)
  printf '{"tree":[]}'
elif [[ "\$args" == *"contents/onboarded-repos.txt"* && "\$args" == *"--fail-with-body"* && "\$args" != *"-X PUT"* && "\$args" != *"-X DELETE"* ]]; then
  # Inventory fetch (uses --fail-with-body): return existing content or 404
  if [[ -n "${existing_b64}" ]]; then
    printf '{"content":"${existing_b64}","sha":"existingfilesha"}'
  else
    exit 1
  fi
elif [[ "\$args" == *"contents/onboarded-repos.txt"* && "\$args" != *"--fail-with-body"* && "\$args" != *"-X PUT"* ]]; then
  # Seed file SHA fetch during init (no --fail-with-body, no PUT/DELETE)
  printf '{"sha":"seedfilesha"}'
...
# ── Run 4: init — inventory written via Contents API, content is correct ──────
rm -f "$INV_CONTENTS_LOG"
...
printf '%s' "$INV_CONTENT4" | grep -q "^# app-id:${TEST_APP_ID}$" \
  && ok  "inventory init: Contents API payload has correct app-id header" \
  || fail "inventory init: Contents API payload has correct app-id header"

printf '%s' "$INV_CONTENT4" | grep -qx "${TEST_OWNER}/repo1" \
  && ok  "inventory init: Contents API payload contains repo1" \
  || fail "inventory init: Contents API payload contains repo1"
